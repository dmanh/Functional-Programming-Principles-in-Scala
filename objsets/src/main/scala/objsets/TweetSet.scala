...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  /*
   * pay attention to the efficiency here! 
   * (using remove function to iterate one by one is not a good approach)
   */
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
...
object GoogleVsApple {
  ...
  def generateTweets(keywords:List[String],acc:TweetSet): TweetSet= 
    if(keywords.isEmpty) acc
    else  generateTweets(keywords.tail,acc union allTweets.filter(p => p.text.contains(keywords.head)))
  
  lazy val googleTweets: TweetSet = generateTweets(google,new Empty)
  lazy val appleTweets: TweetSet = generateTweets(apple,new Empty)
